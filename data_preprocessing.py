...
class DataRetrieval:
    ...
    def _get_daily_data_norm(self, symbol):
        ...
        min_max = daily_price[cols].stack()
        minn = min_max.min()
        maxx = min_max.max()


        def normali(value, low=6000, high=6300):

            gap = high-low
            norm = ( (value-minn) / (maxx-minn)) * gap
            norm = float(round(norm)) + low
            return norm


        for col in cols:
            daily_price[col] = daily_price.apply(lambda x: normali(x[col]), axis=1)

        daily_price['Volume'] = daily_price.apply(lambda x: round(x['Volume']/10000), axis=1)
    ...
    def _dji_components_data(self):
        ...
        for i in DJI + CONTEXT_DATA:
            ...
            daily_price = self._get_daily_data_norm(i) if i in DJI else self._get_daily_data(i)
            if i == (DJI + CONTEXT_DATA)[0]:
                ...
                self.components_df_c[i] = daily_price["Close"]
...
class MathCalc:
    ...
    def calc_return(period):
        ...
        """
        period_return = period - period.shift(1)
    ...
    @staticmethod
    def calc_gain_to_pain(asset_ratio_seq):
        """
        This function computes the gain to pain ratio given a series of cummulative returns
        """
        GPR_MAX = 10
        sum_all = asset_ratio_seq.sum ()

        if sum_all == 0:
            GPR = 0
        else:
            sum_neg = abs(asset_ratio_seq[asset_ratio_seq < 0].sum ())
            GPR = GPR_MAX if sum_neg == 0 else sum_all / sum_neg


        return min(GPR, GPR_MAX)
    ...
    def calc_kpi(portfolio):
        ...
        kpi = pd.DataFrame(index=['KPI'], columns=['Avg. monthly return',  'Avg yearly return'])
